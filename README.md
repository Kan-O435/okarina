...
MIDIキーボードで `C → D → E → G` を2秒以内に続けて弾き、2秒間何も弾かずに待つと、
Consoleに

```
[music] performance ended
[music] melody recorded: [C D E G]
[music] recognized: SUN_MELODY
```

と表示されれば、旋律の記録・認識(`internal/music`)が正しく動作している。
登録されていない旋律を弾いた場合は `recognized: (no match)` と表示される。

### 8. オカリナの発音の確認

「オカリナの音」欄の「音を有効にする」ボタンを一度クリックしてから
(ブラウザの自動再生ポリシーにより、ユーザー操作なしでは音が鳴らないため)、
MIDIキーボードで鍵盤を弾くと、対応する音の高さでオカリナ風の音(サイン波)が鳴る。
「テスト再生」ボタンでMIDIキーボードなしでも動作確認できる。
...
- [x] 取得したMIDIイベントをもとにメロディ判定を行う(internal/music)
- [x] オカリナの発音を実装する(Web Audio API)
- [ ] 認識した旋律でゲーム世界の状態を変更する(昼夜・天候・扉)
