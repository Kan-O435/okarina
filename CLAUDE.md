...
# 30. Claude Codeへの実装ルール

Claude Codeは本プロジェクトの実装を支援する。
...
* `main`には直接コミットしない。必ずfeatureブランチを切り、PR経由でマージする
  (マージ・pushはユーザー自身が行う。Claude Codeはローカルコミットまで)
* コミットメッセージにGitHub Issue番号(`#N`)を書く前に、必ず
  `gh issue view <N> --repo Kan-O435/okarina` でタイトル・本文が対応する内容か確認する。
  本リポジトリはepic Issue本文の中に独自の番号付き下位タスクリスト(例: epic1本文中の
  「#3」)を持つことがあり、これは同じ番号の独立したIssue(例: 独立Issue #3)とは
  別物なので特に注意する。対応が曖昧な場合は、Issue番号を書かずコミットするか、
  ユーザーに確認する
* 作業前に`git fetch`/`git pull`で`dev`(または作業元ブランチ)を最新化してから
  新しいブランチを作る(チームメイトの変更と衝突しやすいため)
