...
func GrasslandObstacleBlocks(prevZ, newZ float64, jumping bool) (blockedZ float64, blocked bool) {
	...
	if newZ < prevZ {
		// 前進(Zが減る方向)して障害物に入った → 手前の境界で止める。
		return GrasslandObstacleNearZ, true
	}
	// 後退(Zが増える方向)して障害物に入った → 奥の境界で止める。
	return GrasslandObstacleFarZ, true
}
